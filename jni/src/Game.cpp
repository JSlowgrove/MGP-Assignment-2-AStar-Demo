	...
	: JAM_State(stateManager, renderer, screenWidth, screenHeight)
{
	...
	border = new JAM_Texture(renderer, 0, 0, 0);
	...
	aStar = new JAM_AStar((int)(screenWidth / squareScale), (int)(screenHeight / squareScale), (float)squareScale, (float)squareScale);
					...
					if (placementType == -1)
					{
						...
						if (mapSquares[(int)mouse.x / squareScale][(int)mouse.y / squareScale]->getDanger())
						{
							...
							mapSquares[(int)mouse.x / squareScale][(int)mouse.y / squareScale]
								...
								(float)squareScale,
								false);
							aStar->setSafeNode((int)mouse.x / squareScale, (int)mouse.y / squareScale);
						}
						else
						{
